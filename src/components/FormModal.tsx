...
export function FormModal({ visible, title, subtitle, submitLabel, canSubmit = true, onClose, onSubmit, children }: FormModalProps) {
  return (
      ...
      <KeyboardAvoidingView style={styles.overlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <View style={styles.panel}>
          ...
          <View style={styles.footer}>
            ...
            <View style={!canSubmit ? styles.disabled : undefined}><Button label={submitLabel} icon="checkmark" onPress={canSubmit ? onSubmit : undefined} /></View>
          </View>
        </View>
...
export function FormField({ label, value, placeholder, onChangeText, multiline = false, keyboardType = 'default' }: { label: string; value: string; placeholder: string; onChangeText: (value: string) => void; multiline?: boolean; keyboardType?: 'default' | 'email-address' }) {
  return (
...
const styles = StyleSheet.create({
  ...
  body: { padding: 22, gap: 18 }, footer: { padding: 18, borderTopWidth: 1, borderTopColor: colors.border, flexDirection: 'row', justifyContent: 'flex-end', gap: 9 }, disabled: { opacity: 0.45 },
